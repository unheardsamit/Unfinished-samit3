// set bash title
process.stdout.write("\x1b]2;Goat Bot V2 - Made by NTKhang & FCA fix by ArYAN\x1b\x5c");
...
const author = ("FCA Fix by ArYAN AHMED RUDRO");
const srcUrl = ("Source code: https://github.com/itzaryan008/Goat-Bot-V2");
const fakeRelease = ("ALL VERSIONS NOT RELEASED HERE ARE FAKE");
for (const t of subTitleArray) {
	const textColor2 = gradient("#9F98E8", "#AFF6CF")(t);
	centerText(textColor2, t.length);
}
centerText(gradient("#0000FF", "#AFF6CF")(author), author.length);
centerText(gradient("#0000FF", "#AFF6CF")(srcUrl), srcUrl.length);
centerText(gradient("#FF00FF", "#f12711")(fakeRelease), fakeRelease.length);
...
async function startBot(loginWithEmail) {
	console.log(colors.hex("#FF00FF")(createLine("START LOGGING IN", true)));
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			logColor("#0000FF", createLine("BOT INFO"));
			...
			let notification;
			try {
				const getNoti = await axios.get("https://raw.githubusercontent.com/itzaryan008/master/refs/heads/main/notification.txt");
			...
			if (global.GoatBot.config.dashBoard?.enable == true && dashBoardIsRunning == false) {
				logColor('#FF00FF', createLine('DASHBOARD'));
				try {
			...
			logColor("#0000FF", createLine("COPYRIGHT"));
			// —————————————————— COPYRIGHT INFO —————————————————— //
			...
			console.log(`\x1b[1m\x1b[33m${("COPYRIGHT:")}\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36m${("Project GoatBot v2 created by NTKhang please do not sell this source code or claim it as your own. Thank you!")}\x1b[0m`);
			logColor("#0000FF", character);

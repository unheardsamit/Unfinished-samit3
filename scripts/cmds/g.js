...
const API_KEY = "AIzaSyCnuhpDQAz7HCPw1O3Ri8O7RDevB0fUFpg"; // Put your API key here
...
async function getTextGemini(uid, prompt = "", fileUrls, reply) {
  ...
  const textResponse = generateContentResponse?.data?.candidates?.[0]?.content?.parts?.[0]?.text || "";

  const modelMessage = { role: "model", parts: [{ text: textResponse }] };
...
module.exports = {
  config: {
    ...
    role: 0,
    description: {
      en: "Fast input text and images with Google Gemini 1.5 flash. Up to 1M context window."
    },
    guide: { en: "{pn} <query>" },
    category: "ai",
  },
  onStart: async function ({ api, message, event, args, commandName }) {
    ...
    if (prompt.toLowerCase() === "clear") {
      ...
      message.reply("চ্যাট হিস্ট্রি সফলভাবে মুছে ফেলা হয়েছে। UID: " + uid);
      return;
    }

    var contentTextOnly = (event.type == "message_reply") ? event.messageReply.body : args.join(" ");
    var contextImageOnly = (event.type == "message_reply") ? event.body : args.join(" ");
